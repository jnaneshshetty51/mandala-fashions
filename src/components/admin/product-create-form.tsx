...
export function ProductCreateForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (state.isSubmitting || state.isRedirecting) return;

    // Capture form reference immediately — event.currentTarget becomes null after any await
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setState({
          isSubmitting: false,
          isRedirecting: false,
          error: firstFieldError ?? result?.error ?? "Unable to create product right now.",
          success: null
        });
        return;
      }
      ...
      if (result?.data?.id) {
        setState({
          ...
          isRedirecting: true,
          error: null,
          success: "Product created. Redirecting to editor..."
        });
